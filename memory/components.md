...
- **`app/prestataires/`** — carnet : CRUD par l'auteur, delete aussi par référent, recherche client-side `normalizeSearch`. Cards en grille `grid-cols-1 md:grid-cols-2` (`items-start`), tuile d'icône `Wrench` en ancre visuelle, métier en badge, contacts en pastilles tactiles. Le retour d'expérience n'est **pas** tronqué : il n'existe pas de page de détail prestataire. Push `new_provider` à la création.
...
### `EventActions.tsx` — LE bloc d'actions d'un événement (unique)
Modifier + Supprimer (confirmation en deux temps), pour le **créateur ou un référent** (libellés suffixés « (référent) », policy 037). Rend `null` sans droits — les parents ne portent aucune condition de rôle. Suppression via `deleteEventWithImages()` (`lib/events.ts` : RLS arbitre, **contrôle 0-ligne** — un delete refusé par RLS ne renvoie pas d'erreur —, ligne puis images). Props : `event`, `onDeleted(id)` (navigation ou retrait de liste, au choix du parent), `variant: 'stacked' | 'row'`.

**Règle : ne jamais ajouter une action d'événement directement dans un écran.** Trois consommateurs : `EventDetailClient` (page, `onDeleted` → `router.push('/evenements')`), `EventDetailPopup` (panneau de la liste desktop, `onDeleted` → filtre `EventsList` + `onClose`), `ProfileClient` (« mes événements », variante `row`, `onDeleted` → filtre local). La duplication de ce bloc a causé deux bugs de divergence avant sa centralisation (2026-08-03).

Chaîne référent-édition : policy `events_update` (037) + la page `evenements/[id]/edit` ne filtre plus sur `user_id` (contrôle owner-ou-référent côté serveur) + l'update d'`EventForm` sans `.eq('user_id')` avec contrôle 0-ligne. Le payload d'update ne contient pas `user_id` : un référent qui édite ne s'approprie pas l'événement.  
